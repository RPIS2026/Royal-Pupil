...
export default function Faculty() {
  const leadership = [
    {
      ...
      positions: [
        { title: "President", org: "Sri Sai Ram Trust, Kudatini" },
        {
          title: "Director",
          org: "Royal Pupil International Residential School, Kudatini",
        },
        {
          title: "Managing Partner",
          org: "Janahitha Multispeciality Hospital, Ballari",
        },
        { title: "MD", org: "R S Builders, Ballari" },
      ],
      details: [
        "Born to Smt. C. Hymavathi and Late Sri C. Aswath Narayana Reddy; schooled at Bala Bharathi School and Wardlaw High School / Wardlaw Composite Junior College, Ballari.",
        "Completed MBBS from M R Medical College, Gulbarga, in 2001.",
        "Began his medical career as Casualty Medical Officer at OPEC Apollo Hospital (Raichur), St. Mary Hospital (Anchal, Kerala), VIMS TB Sanitorium (Ballari) and Jindal Sanjeevani Hospital (Toranagallu).",
        "Took on hospital administration in 2014, managing R K Hospital, Ballari.",
        "Founded Janahitha Multispeciality Hospital and Royal Pupil International Residential School, Kudatini, in 2018, followed by R S Builders in 2025.",
      ],
    },
    {
  ...
  return (
    <section
      id="faculty"
      className="scroll-mt-[72px] py-[40px] px-[40px] bg-rp-cream w-full max-w-7xl mx-auto"
    >
            ...
            >
              <div className="flex gap-[16px] mb-[12px]">
                ...
                <div>
                  ...
                  <p className="text-rp-gold font-medium tracking-wide text-[11px] uppercase">
                    {person.role}
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <p className="text-rp-dark/80 text-[12px] leading-[1.4] mb-4">
                  {person.bio}
                </p>

                {person.positions && (
                  <div className="bg-rp-mint/10 rounded-md p-3 mb-4 text-[11px] leading-relaxed text-rp-dark/90">
                    {person.positions.map((pos, pIdx) => (
                      <span key={pIdx}>
                        <strong>{pos.title}</strong> — {pos.org}
                        {pIdx < person.positions.length - 1 && (
                          <span className="mx-1 text-rp-sage">·</span>
                        )}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {person.badges.map((badge, bIdx) => (
                    <span
                      key={bIdx}
                      className="bg-rp-mint/20 text-rp-leaf px-2 py-0.5 rounded text-[10px] font-bold border border-rp-mint"
                    >
                      {badge}
                    </span>
                  ))}
                ...
                <div>
                  <ul className="space-y-1.5">
                    {person.details.map((item, iIdx) => (
                      <li
                        key={iIdx}
                        className="flex gap-2 text-[11px] text-rp-dark/80"
                      >
